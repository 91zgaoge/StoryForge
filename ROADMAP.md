...
> 最后更新: 2026-08-15（v0.47.0 续写质量闭合：债务/节点/阵容/状态网）

## ✅ v0.27.x–v0.47.x 已实施完成
...
- **真机验收与学习飞轮调优**：三代理创世/续写的真机盲测；**v0.47.0 设计 §8.2 连续 8 次幕前续写真机探针尚未跑**（需 LLM；CI 仅有 `eight_beat_append_quality_contract` mock），未通过前不得宣称人物丢失/错配、情节推进缓慢、前后文断裂三症状已修复；质量门阈值与 grader 权重校准；持续学习双轨（观察 → instinct → 晋升技能）的置信度参数与晋升质量跟踪。
...
- **续写上下文膨胀（注入层已收口，表内未收口）**：Agency 续写已按拍筛选角色卡/名单/大纲去重/单场前文（待发版）。`story_outlines` 仍无界追加、跨故事脏角色行不删除；注入层忽略不能替代表内收口。
