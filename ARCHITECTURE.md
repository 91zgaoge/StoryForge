# StoryMoss (草苔) v0.30.32 架构文档

> **v0.30.32**：增强性指令纳入世界观/故事大纲/场景大纲/上下文强关联。v0.30.31 让世界观/故事大纲/场景大纲/进度彼此强关联（`build_progression_anchor`），但用户的增强性指令（logline hint 后缀）未纳入这套强关联--增强后缀生成时不读世界观，进入管线后又与资产各居一隅、无调和指令。两个根因修复：①`commands/orchestrator.rs` `build_logline_context_sync` 新增拉 `world_buildings`（concept+rules前3+history 截断1000）为 `world_setting` 注入 `LoglineContext`，`agency_logline_suffix_contextual.md` 新增 `## 世界观设定` 段 + 输出要求"后缀须与世界观规则一致，不得提出违反世界观的设定或角色"；②`agents/orchestrator.rs` `build_progression_anchor` 签名加 `user_instruction: &str`，指令非空时作为首个段【本次创作指令（你的创作方向）】注入，资产段标签加"硬约束："前缀，收尾指令改为三分支显式调和（指令+资产：资产=硬约束/指令=创作方向/在硬约束内落实指令核心意图/冲突时调整指令具体表现以符合约束但保留核心意图/不得因约束丢指令也不得因指令违反约束；仅指令无资产：推进剧情向前发展；仅资产无指令：保留原推进约束）。创世 `coordinator.rs` `writer_first_chapter`/`writer_prose_fallback`（补齐此前缺失的"资产为准"）与 TimeSliced `orchestrator_timesliced_writer.md`/fallback 字符串同步补调和指令。
