...
# StoryForge (草苔) v4.1.0 - AI 导演式小说创作系统
...
>
> **v4.1.0 最新更新**：幕前界面深度重构 — 化整为零，萤火随行。从 20+ 可见 UI 元素缩减至 <5 持久元素，AI 功能以萤火暗示形式按需浮现。44px 极简顶栏、底栏删除、内联 `/` 命令菜单、幽灵文本、三态文思模式。
...
- **顶部走马灯提示栏** - 22px 细条，循环滚动展示所有快捷键提示（`Ctrl+Enter` 续写 / `Ctrl+Space` 文思 / `F11` 禅模式 / `/` 命令菜单 / `Tab` 接受 `Esc` 拒绝），hover 暂停，禅模式自动隐藏
- **AI 萤火随行** - `Ctrl+Space` 循环三态文思模式：`off·`（关闭）/ `passive✨`（被动暗示）/ `active🔥`（主动续写）
- **幽灵文本续写** - `Ctrl+Enter` 触发 AI 续写，结果以灰色斜体幽灵文本呈现于编辑器末尾，`Tab` 接受 / `Esc` 拒绝
- **内联 `/` 命令菜单** - 编辑器内输入 `/` 触发，9 命令：续写/润色/古风/场景/自动续写/审校/评点/排版/自由指令
- **自由指令创作** - WenSiPanel 浮动面板新增"自由指令"标签页，可输入任意创作指令（如"写一篇武侠小说吧"），发送后 AI 基于当前章节上下文执行
...
**P0 核心重构**
- 顶栏精简为 44px 细线 + 22px 走马灯提示栏：小说标题/章节信息/字数/字号/文思三态/禅模式。所有快捷键提示集中于走马灯栏循环展示，hover 暂停
