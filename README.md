...
>
> **v4.3.0 最新更新**：智能交互创作流程深度优化 — 从"能创作"到"懂创作"。一键创建小说后前端实时显示5步进度并自动加载第一章；PlanGenerator全面模型化，彻底移除关键词匹配，LLM自由理解意图；新增设定修改智能响应（角色/世界观/场景自动更新并标记重写）；CapabilityRegistry扩展MCP工具和查询能力；PlanContext注入世界观、角色、伏笔、风格DNA等完整上下文，续写时真正"记得"故事的一切。
>
