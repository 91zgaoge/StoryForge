...
<p align="center">
  <a href="./CHANGELOG.md"><img alt="Version" src="https://img.shields.io/badge/version-v0.53.5-gold"></a>
...
### v0.53.5 · 大纲生成后可改再保存，也可重写

输入「写后续的故事大纲，同时生成后续的场景大纲」后弹出可编辑对话框：确认写入幕后，取消废弃，重写再生成一轮。纸面不改。请升级到 0.53.5 并重启后再试原句。这不是续写质量修复。

### v0.53.4 · 「写后续的故事大纲」不再往正文里加字

输入「写后续的故事大纲，同时生成后续的场景大纲」会重写幕后大纲并弹出结果，不会当成续写。请升级到 0.53.4 并重启后再试原句。这不是续写质量修复。

### v0.53.3 · 「重新写后续大纲」会弹出写好的大纲，不改正文

输入「根据正文内容重新写后续的故事大纲，同时生成场景大纲」后，幕前弹出「已按正文重写设定」，里面是刚写入幕后的故事大纲和场景大纲。纸面一个字都不会多。请升级到 0.53.4 并重启后再试原句。这不是续写质量修复。

### v0.53.2 · 「重新生成大纲」和场景大纲不再因没花括号丢掉

管理模型用 `story_outline:` / `scene_outline：` 这种写法时也会写入幕后。请升级到 0.53.4 并重启后再试原句。这不是续写质量修复。

### v0.53.1 · 按正文重写大纲不再因 JSON 形状丢掉

管理模型把大纲做成对象或直接写成一段话时，现在会落进幕后故事大纲，而不是报「未能解析设定 JSON」。请升级到 0.53.4 并重启后再输入原句。这不是续写质量修复。

### v0.53.0 · 按正文重写大纲，不再往纸面里加字

幕前输入「将故事大纲按照现有正文重新写过」，会按已写章节重写幕后故事大纲，正文一个字都不会多。角色、世界观、场景大纲把名字写进指令即可。手改过的角色卡默认不覆盖。请升级到 0.53.4 并重启。这不是续写质量修复。

### v0.52.0 · 续写看近 1500 字，并记下为什么选中这些人

点续写时，刚写完的人和债不再因为窗口太短被漏掉。角色卡按需要给全卡或半卡，故事大纲不再越攒越长。请升级到 0.52.0 并重启。同一开头再续写的真机还没跑，不得当成续写质量已修好。

### v0.51.6 · 分给三个代理的模型不再挤到创作模型上

幕后把创作/工具/后台设成不同模型后，管理和资产回流不再抢主创那一台。请升级到 0.51.6 并重启。

### v0.51.5 · 进行中的续写不再弹前台中断卡

续写进行中再点一次，不再挡住正文。底栏「Agency 续写中」结束或点取消后再续写即可。请升级到 0.51.5 并重启。

### v0.51.4 · 「已有创作任务」不再打发去设置

续写进行中再点一次，会提示这部作品正在写，点「知道了」等底栏结束，或「取消当前续写」。请升级到 0.51.4 并重启。

### v0.51.3 · 续写不再用规划把 600 秒耗尽

本地模型若把「故事大纲归纳」写进设定，下一拍会跟着写规划，写完被丢掉再重试直到前端超时。现已在喂给主创前切掉这类提纲，时间不够也不再开第二轮。请升级到 0.51.3 并重启。

### v0.51.2 · 续写不再把节拍规划写进正文

本地模型做主创时，有时会把「本拍任务 / 状态网 / 约束」分析写进章节。现已在落库前切断这类泄露并重试。请升级到 0.51.2 并重启。

### v0.51.1 · 生成中的取消键不再是系统凸起灰块

续写进行时，底栏右侧 X 与顶栏设置键同一套扁平墨纸：透明底、无边无影，悬停淡陶土。请升级到 0.51.1 并重启。

### v0.51.0 · 幕前写一段，工作室三个代理会跟着看

手写或粘贴正文并停手约 30 秒后，幕后「代理工作室」会出现一条「观察」记录：管理整理人物设定，主创根据已写正文准备当前场大纲和下一拍，编辑审查但不改你的正文。请升级到 0.51.0 并重启。字数一次增加不够约 200 字时不会重跑。

测试：`cargo test --lib` 1463 passed / 2 ignored（+9）；`npx vitest run` 592 passed / 3 skipped（+1）。
...
章节已经打开、底栏也在续写，却弹出要你先开章节。这是自动分章把章节 id 当成了场景 id。请升级到 0.50.1 并重启；当前这章可以再点一次章节名，或直接再续写。

测试：`cargo test --lib` 1450 passed / 2 ignored（+1）；`npx vitest run` 590 passed / 3 skipped。
