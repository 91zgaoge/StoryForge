...
use crate::{
    audit::AuditService,
    creative_engine::auto_rewrite_executor::{AuditIssue, AuditSeverity, AutoRewriteExecutor},
    ...
    async fn run_audit_inner(&self, payload: &AuditPayload) -> Result<u32, String> {
        ...
        if !high_issues.is_empty() {
            ...
            tokio::spawn(async move {
                ...
                AutoRewriteExecutor::process_audit_results(
                    auto_handle,
                    auto_pool,
                    &high_audit_issues,
                    &low_audit_issues,
...
```"#;
    ...
    fn timeline2_annotation_create_and_query_roundtrip() {
        use crate::db::{connection::create_test_pool, TextAnnotationRepository};
