...
pub struct ContentAnalysis {
    ...
    /// 世界观增量（仅当本段内容揭示了新的世界观设定时输出）
    #[serde(default)]
    pub world_building: Option<WbDelta>,
    /// 本场景大纲（从正文中还原的场景级纲要）
    #[serde(default)]
    pub scene_outline: Option<SceneOutlineDelta>,
    /// 故事大纲增量（核心冲突/转折点）
    #[serde(default)]
...
pub struct WbDelta {
    ...
    pub concept: String,
    #[serde(default)]
    pub rules: Vec<WbRuleDelta>,
    #[serde(default)]
    pub history: String,
    #[serde(default)]
    pub cultures: Vec<WbCultureDelta>,
}

/// 世界规则增量
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct WbRuleDelta {
    #[serde(default)]
    pub name: String,
    #[serde(default)]
...
pub struct AnalyzedEntity {
    ...
    #[serde(default)]
    pub age: Option<i32>,
    /// 情感内核：主导情感倾向（仅 Character）
    #[serde(default)]
    pub emotional_core: String,
    /// 情感触发：什么引发强烈反应（仅 Character）
    #[serde(default)]
    pub emotional_trigger: String,
    /// 情感创伤：驱动行为的过往创伤（仅 Character）
    #[serde(default)]
    pub emotional_wound: String,
    /// 情感需求：内心真正渴望（仅 Character）
    #[serde(default)]
    pub emotional_need: String,
    /// 该实体在本段内容中的重要性（0.0-1.0）
...
pub struct AnalyzedRelation {
    ...
    #[serde(default)]
    pub dynamic: String,
    /// source 对 target 的情感纽带
    #[serde(default)]
    pub emotional_bond: String,
    /// source 对 target 的情感强度（0.0-1.0）
    #[serde(default = "default_strength")]
    pub emotional_intensity: f32,
    /// target 对 source 的情感纽带
    #[serde(default)]
    pub reverse_emotional_bond: String,
    /// target 对 source 的情感强度（0.0-1.0）
    #[serde(default = "default_strength")]
...
impl IngestPipeline {
    ...
    async fn run_ingest(
        ...
        if let Some(pool) = &self.pool {
            ...
            if synced > 0 {
                log::info!(
                    "[Ingest] 资产回流同步 {} 项到生产资产表 (story_id={})",
                    synced,
                    content.story_id
                );
            }
...
{{
  "entities": [
    {{
      ...
      "importance_score": 0.9 // 0.0-1.0，该实体在本段内容中的重要性
    }}
  ],
  "relationships": [
    {{
      ...
      "emotional_intensity": 0.9, // 0.0-1.0 的浮点数
      "reverse_emotional_bond": "target 对 source 的情感纽带",
      "reverse_emotional_intensity": 0.7 // 0.0-1.0 的浮点数
    }}
  ],
  "events": [
    {{
  ...
  "world_building": {{
    ...
    "rules": [{{"name": "规则名", "description": "规则描述", "rule_type": "magic/technology/social/physical/biological/historical/cultural/custom 之一", "importance": 7}}],
    "history": "历史背景增量",
    "cultures": [{{"name": "文化名", "description": "描述", "customs": ["习俗1"], "values": ["价值观1"]}}]
  }},
  "scene_outline": {{
...
3. 角色画像字段（role_type/personality/background/goals/fears/appearance/gender/age/4个 emotional_*）仅对 entity_type=Character 填写，且必须有文本依据，无依据则省略
4. 关系必须有明确的原文证据支持，禁止臆测
